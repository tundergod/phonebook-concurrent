...
static thread_t* least_load_schedule(tpool_t *tpool)
{
    ...
    for (i = 1; i < tpool->num_threads; i++) {
        if (thread_queue_len(&tpool->threads[i]) < thread_queue_len(&tpool->threads[min_num_works_index]))
...
static void *tpool_thread(void *arg)
{
    ...
    __sync_fetch_and_add(&global_num_thread, 1);//返回更新前的值
    pthread_kill(main_tid, SIGUSR1);
    sigemptyset(&zeromask);//將信號集初始化並清空,成功返回0;
    sigemptyset(&newmask);
    sigaddset(&newmask, SIGUSR1);//将参数signum代表的信号加入至参数set 信号集里.执行成功则返回0，如果有错误则返回-1。
...
static void spawn_new_thread(tpool_t *tpool, int index)
{
    ...
    if (pthread_create(&tpool->threads[index].id, NULL, tpool_thread, (void *)(&tpool->threads[index])) != 0) {
...
static int dispatch_work2thread(tpool_t *tpool, thread_t *thread, void(*routine)(void *), void *arg)
{
    tpool_work_t *work = NULL;
